...
export class AiService {
    ...
    async generateBusinessReply(params: {
        ...
        const { businessContext, knowledgeSnippets, messages } = params;
...
`.trim();
        ...
        const kbText = knowledgeSnippets.length
          ? knowledgeSnippets
            .map(
              (s, i) => `[${i + 1}] Source: ${s.source}\n${s.content}`,
            )
            .join('\n\n')
          : 'Немає релевантних фрагментів, відповідай на основі загальних знань і бізнес-контексту.';
      ...
      label: string,
        ...
        return new Promise<T>((resolve, reject) => {
            const timer = setTimeout(
              () =>
                reject(
                  new Error(`Timeout in ${label} after ${ms}ms`),
                ),
              ms,
            );
    ...
    async embedQuery(query: string): Promise<number[] | null> {
        ...
        try {
            ...
            const embedding =
              response.data[0]?.embedding as number[] | undefined;
